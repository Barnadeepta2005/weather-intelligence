...
export default function Page() {
  ...
  const fetchWeatherForLocation = useCallback(async (targetLocation: SelectedLocation) => {
    ...
    try {
      ...
      if (err instanceof Error && err.name === 'AbortError') return // Silent cancellation for superseded request
      if (requestId === activeRequestRef.current) {
        const message = err instanceof Error ? err.message : 'Failed to retrieve live weather data.'
        setError(message)
        setData(null)
      }
    } finally {
